...
async function handleLogin() {
    ...
    if (typeof db === "undefined" || !db) { alert("Firebase bağlantısı kuruluyor..."); return; }
    try {
    ...
    } catch (error) { console.error("Giriş hatası:", error); alert("Teknik bir hata oluştu."); }
}
...
async function handleRegister() {
    ...
    if (!inviteCode || !nickname || !email || !password || !passwordConfirm) { alert("Tüm alanları doldurun!"); return; }
    if (!validateNickname(nickname)) { alert("Kullanıcı adı 3-20 karakter, sadece harf/rakam!"); return; }
    if (password !== passwordConfirm) { alert("Şifreler uyuşmuyor!"); return; }
    if (password.length < 4) { alert("Şifre en az 4 karakter!"); return; }
    ...
    if (userList.some(u => u.email === email)) { alert("Bu e-posta zaten kayıtlı!"); return; }
    if (userList.some(u => u.nickname === nickname)) { alert("Bu kullanıcı adı zaten alınmış!"); return; }
    const newUserKey = email.replace(/\./g, ',');
    await fbSet(`ladesUsers/${newUserKey}`, { email, password, nickname, balance: 0, isAdmin: false, createdAt: Date.now() });
    const inviteKey = Object.keys(inviteCodes).find(k => inviteCodes[k] === inviteCode);
    if (inviteKey) await fbRemove(`inviteCodes/${inviteKey}`);
    alert(`✅ Kayıt başarılı, ${nickname}! 🎉 Başlangıç bakiyeniz: 0 TOKEN`);
...
async function requestInviteCode() {
    ...
    if (existing) { alert("Zaten açık talebiniz var."); return; }
    ...
    if (typeof sendNotificationToAdmins === 'function') {
        await sendNotificationToAdmins({ title: "📩 Davet Kodu Talebi!", message: `${email} davet kodu talep ediyor!`, type: "invite_request" });
    }
    alert("✅ Davet kodu talebiniz yöneticiye iletildi!");
}
...
function filterCategory(categoryName) {
    ...
    if (typeof fbGet === "function") {
        fbGet("customMarkets").then(marketsObj => { renderMarketGrid(marketsObj || {}); }).catch(err => console.error(err));
    }
}
...
function renderLeaderboard(usersObj) {
    ...
    const sortedUsers = Object.values(usersObj).filter(u => u && u.email && (u.balance || 0) > 0).sort((a, b) => (b.balance || 0) - (a.balance || 0));
    if (sortedUsers.length === 0) {
        leaderboardList.innerHTML = `<div style="text-align:center; color:#64748b; padding:30px; font-size:14px;">Henüz token kazanmış kullanıcı yok. 🏆</div>`;
        return;
    }
...
function generateMarketCardHTML(market, isActive) {
    ...
    return `
        <div class="market-card" style="position: relative; ${!isActive ? 'opacity: 0.9; border-color: #1c2541; background: #060b19;' : ''}">
            ${adminDeleteHTML}
            <div class="market-info" onclick="toggleMarketDetail('${market.id}')">
                ...
                <div style="display:flex; gap:8px; align-items:center; margin-top:6px;">
                    ...
                    <button onclick="event.stopPropagation(); toggleMarketDetail('${market.id}')" 
                        ...
                        <i class="fa-solid fa-chevron-down" id="detail-toggle-${market.id}"></i> Detaylar
                    </button>
                </div>
                <div id="detail-content-${market.id}" class="market-detail-content" style="display: none; margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(28,37,65,0.5);">
...
function renderMarketGrid(marketsObj) {
    ...
    if (marketGrid) {
        ...
        if (activeMarkets.length === 0) {
            marketGrid.innerHTML = `<div style="text-align:center; color:#64748b; padding:40px; width:100%;">Şu an bu kategoride aktif lades yok.</div>`;
        } else {
            activeMarkets.forEach(market => { marketGrid.innerHTML += generateMarketCardHTML(market, true); });
        }
    }
    if (pastMarketGrid) {
        pastMarketGrid.innerHTML = "";
        if (pastMarkets.length === 0) {
            pastMarketGrid.innerHTML = `<div style="text-align:center; color:#64748b; padding:40px; width:100%;">Henüz sonuçlanmış lades yok.</div>`;
        } else {
            pastMarkets.forEach(market => { pastMarketGrid.innerHTML += generateMarketCardHTML(market, false); });
        }
    }
...
async function loadBetParticipants(marketId) {
    ...
    try {
        ...
        const marketBets = Object.values(history).filter(b => b && b.marketId === marketId).sort((a, b) => (b.amount || 0) - (a.amount || 0));
        ...
        const userPromises = marketBets.map(async (bet) => {
            ...
            return { nickname, choice: bet.choice, amount: bet.amount };
        });
...
async function createNewMarket() {
    ...
    if (typeof db === "undefined" || !db) { alert("Firebase bağlantısı yok."); return; }
    const MIN_BET = 250, MAX_BET = 1000;
    if (isNaN(initialBet) || initialBet < MIN_BET) { alert(`❌ Minimum ${MIN_BET} Token yatırmanız gerekiyor!`); return; }
    if (initialBet > MAX_BET) { alert(`❌ Maksimum ${MAX_BET} Token yatırabilirsiniz!`); return; }
    ...
    if (!title || !date) { alert("Lütfen alanları doldurun!"); return; }
    if (category !== "Spor" && choice === "DRAW") { alert("Beraberlik sadece Spor kategorisinde!"); return; }
    ...
    if (typeof sendNotificationToAllUsers === 'function') {
        await sendNotificationToAllUsers({
            title: "📢 Yeni Lades!", message: `${currentUser.nickname || maskUserEmail(currentUserEmail)}, "${title}" ladesini oluşturdu!`,
            type: "new_market", marketId: marketId, link: "dashboard.html?tab=mevcut-ladesler"
        });
    }
    alert(`⚡ Lades Başarıyla Yaratıldı! 💰 ${initialBet.toLocaleString("tr-TR")} Token yatırıldı.`);
    document.getElementById("market-question").value = "";
    document.getElementById("market-initial-bet").value = "";
    const tabButtons = document.querySelectorAll('.tab-button');
    tabButtons.forEach(btn => { if (btn.innerText.includes('Mevcut Ladesler')) btn.click(); });
}
...
async function confirmBet() {
    ...
    const currentUserEmail = localStorage.getItem("currentUser");
    if (!currentUserEmail) { alert("Lütfen giriş yapın!"); window.location.href = "login.html"; return; }
    if (typeof db === "undefined" || !db) { alert("Firebase bağlantısı yok."); return; }
    if (isNaN(amount) || amount <= 0) { alert("Geçersiz miktar!"); return; }
    const MIN_BET = 250, MAX_BET = 1000;
    if (amount < MIN_BET) { alert(`❌ Minimum bahis ${MIN_BET} Token!`); return; }
    if (amount > MAX_BET) { alert(`❌ Maksimum bahis ${MAX_BET} Token!`); return; }
    const userKey = currentUserEmail.replace(/\./g, ',');
    const currentUser = await fbGet(`ladesUsers/${userKey}`);
    if (!currentUser) { alert("Kullanıcı bulunamadı!"); return; }
    ...
    if (typeof sendBetNotificationToParticipants === 'function') {
        await sendBetNotificationToParticipants(activeMarketId, currentUserEmail, activeChoice, amount, target.title, currentUser.nickname || maskUserEmail(currentUserEmail));
    }
...
async function finalizeLades(marketId, winningChoice) {
    ...
    const markets = marketsSnap || {};
    const market = markets[marketId];
    if (!market) { alert("❌ Lades bulunamadı!"); return; }
    const yesPool = market.yesPool || 0, noPool = market.noPool || 0, drawPool = market.drawPool || 0;
    const totalPool = market.category === "Spor" ? (yesPool + noPool + drawPool) : (yesPool + noPool);
    let winningPool = 0, winningChoiceName = "";
    if (winningChoice === "YES") { winningPool = yesPool; winningChoiceName = "EVET"; }
    else if (winningChoice === "NO") { winningPool = noPool; winningChoiceName = "HAYIR"; }
    else if (winningChoice === "DRAW") { winningPool = drawPool; winningChoiceName = "BERABERLİK"; }
    if (totalPool === 0 || winningPool === 0) {
        alert(`⚠️ ${winningChoiceName} havuzu boş. Lades kapatıldı.`);
    ...
    if (winners.length === 0) {
        alert(`⚠️ ${winningChoiceName} seçeneğine bahis yapan yok.`);
        market.status = "Sonuçlandı";
        await fbSet(`customMarkets/${marketId}`, market);
        return;
    }
    ...
    if (typeof createNotification === 'function') {
        const resultPromises = allParticipants.map(async (participant) => {
            ...
            const winAmount = isWinner ? distributionResults.find(r => r.email === participant.email)?.reward || 0 : 0;
            ...
            return createNotification(participant.email, {
                title: isWinner ? "🎉 Kazandınız!" : "😔 Kaybettiniz",
                message: isWinner ? `${market.title} ladesinde ${winAmount} Token kazandınız! 🏆` : `${market.title} ladesinde ${participant.amount} Token kaybettiniz.`,
                type: "result", marketId: marketId, link: "profil.html"
            });
        });
        await Promise.all(resultPromises);
    }
    const remainingTokens = totalPool - totalDistributed;
    let distributionDetails = distributionResults.map(r => `  • ${r.nickname}: ${r.amount} Token → ${r.reward} Token kazandı (${r.share})`).join('\n');
    alert(`🎉 ${winningChoiceName} KAZANDI!\n📊 Toplam Havuz: ${totalPool} Token\n💰 Dağıtılan: ${totalDistributed} Token\n👥 Kazanan Sayısı: ${winners.length}\n\n📋 DAĞITIM:\n${distributionDetails}`);
}

async function deleteMarketCompletely(marketId, marketTitle) {
    if (typeof db === "undefined" || !db) { alert("❌ Firebase bağlantısı yok!"); return; }
    if (localStorage.getItem("currentUser") !== "tsulhan@gmail.com") { alert("❌ Sadece yönetici!"); return; }
    if (!confirm(`"${marketTitle}" AKTİF ladesini silip tokenları iade etmek istediğinize emin misiniz?`)) return;
    try {
        ...
        loadingMsg.innerHTML = '⏳ Lades siliniyor...';
        document.body.appendChild(loadingMsg);
        const [betHistorySnapshot, usersSnapshot] = await Promise.all([db.ref("betHistory").once("value"), db.ref("ladesUsers").once("value")]);
        const allHistories = betHistorySnapshot.val() || {};
        const allUsers = usersSnapshot.val() || {};
        const deletePromises = [];
        const userUpdates = { ...allUsers };
        let refundedTokenCount = 0, affectedUsersCount = 0;
        ...
        alert(`✅ "${marketTitle}" başarıyla silindi! 👥 ${affectedUsersCount} kullanıcı, 💰 ${refundedTokenCount} Token iade edildi.`);
...
async function deleteMarketFromHistory(marketId, marketTitle) {
    if (typeof db === "undefined" || !db) { alert("Firebase bağlantısı yok!"); return; }
    if (localStorage.getItem("currentUser") !== "tsulhan@gmail.com") { alert("❌ Sadece yönetici!"); return; }
    if (!confirm(`"${marketTitle}" GEÇMİŞ ladesini silmek istediğinize emin misiniz?`)) return;
    try {
        const loadingMsg = document.createElement('div');
        ...
        if (deletePromises.length > 0) await Promise.all(deletePromises);
        document.body.removeChild(loadingMsg);
        alert(`✅ "${marketTitle}" geçmişten silindi! 🗑️ ${deletedCount} bahis kaydı temizlendi.`);
        setTimeout(() => { location.reload(); }, 1000);
    } catch (error) {
        console.error("❌ Lades silme hatası:", error);
        alert("❌ Lades silinirken hata oluştu.");
        const loadingMsg = document.querySelector('div[style*="position:fixed"]');
        if (loadingMsg) document.body.removeChild(loadingMsg);
    }
}
...
async function openAdminPanel() {
    ...
    if (!modal) { alert("Admin paneli bulunamadı!"); return; }
    ...
    try {
        ...
        if (requestsList) {
            ...
            Object.entries(requests).forEach(([key, req]) => {
                if (req && req.status === "Bekliyor") {
                    ...
                    let actionButtons = "", requestInfo = "";
                    if (isToken) {
                        requestInfo = `<span style="font-size:12px; color:#ff4aa2;">${nickname} (${email}) - 💰 ${amount} Token talep ediyor (Mevcut: ${currentBalance.toLocaleString("tr-TR")} Token)</span>`;
                        actionButtons = `<button onclick="approveToken('${req.id}', '${email}', ${amount})" style="background:#22c55e; color:black; border:none; padding:4px 12px; border-radius:5px; font-weight:bold; cursor:pointer; font-size:12px;">✅ Onayla</button>`;
                    } else {
                        requestInfo = `<span style="font-size:12px; color:#24ffff;">${email} - ✉️ Davet kodu talep ediyor</span>`;
                        actionButtons = `<button onclick="approveInvite('${req.id}', '${email}')" style="background:#22c55e; color:black; border:none; padding:4px 12px; border-radius:5px; font-weight:bold; cursor:pointer; font-size:12px;">🔑 Kod Üret</button>`;
                    }
                    requestsList.innerHTML += `
                        <div style="display:flex; justify-content:space-between; align-items:center; background:#030814; padding:12px 15px; border-radius:8px; margin-bottom:8px; border-left: 3px solid ${isToken ? '#ff4aa2' : '#24ffff'};">
                            <div style="display:flex; flex-direction:column; gap:4px;"><span style="font-size:13px; font-weight:600; color:white;">${nickname}</span>${requestInfo}</div>
                            <div style="display:flex; gap:6px; flex-wrap:wrap;">${actionButtons}<button onclick="deleteRequest('${key}')" style="background:#ef4444; color:white; border:none; padding:4px 12px; border-radius:5px; font-weight:bold; cursor:pointer; font-size:12px;">❌ Reddet</button></div>
                        </div>
                    `;
                }
            });
            if (!hasPending) requestsList.innerHTML = `<div style="text-align:center; color:#64748b; padding:20px; font-size:13px;">✅ Bekleyen talep bulunmuyor.</div>`;
        }
        ...
        if (codesList) {
            ...
            if (Object.keys(codes).length === 0) {
                codesList.innerHTML = `<div style="text-align:center; color:#64748b; padding:10px; font-size:13px; width:100%;">Henüz oluşturulmuş davet kodu yok.</div>`;
            } else {
                Object.entries(codes).forEach(([key, code]) => {
                    codesList.innerHTML += `
                        <div style="background:rgba(36,255,255,0.1); border:1px solid #24ffff; padding:8px 14px; border-radius:8px; font-size:13px; color:#24ffff; display:inline-flex; align-items:center; gap:10px; margin:4px;">
                            <span style="font-family:monospace; font-weight:700;">${code}</span>
                            <i class="fa-solid fa-trash" onclick="deleteInviteCode('${key}')" style="cursor:pointer; color:#ef4444; font-size:14px;" title="Kodu Sil"></i>
                        </div>
                    `;
                });
            }
        ...
        if (adminActiveMarkets) {
            ...
            if (activeMarkets.length === 0) {
                adminActiveMarkets.innerHTML = `<div style="text-align:center; color:#64748b; padding:15px; font-size:13px;">Şu an aktif bir lades pazarı yok.</div>`;
            } else {
                activeMarkets.forEach(m => {
                    const yesPool = m.yesPool || 0, noPool = m.noPool || 0, drawPool = m.drawPool || 0;
                    const total = m.category === "Spor" ? (yesPool + noPool + drawPool) : (yesPool + noPool);
                    let buttons = `
                        <button onclick="finalizeLades('${m.id}', 'YES')" style="background:#22c55e; color:black; border:none; padding:5px 12px; border-radius:5px; font-weight:bold; font-size:11px; cursor:pointer;">EVET</button>
                        <button onclick="finalizeLades('${m.id}', 'NO')" style="background:#ef4444; color:white; border:none; padding:5px 12px; border-radius:5px; font-weight:bold; font-size:11px; cursor:pointer;">HAYIR</button>
                    `;
                    if (m.category === "Spor") {
                        buttons = `
                            <button onclick="finalizeLades('${m.id}', 'YES')" style="background:#22c55e; color:black; border:none; padding:5px 12px; border-radius:5px; font-weight:bold; font-size:11px; cursor:pointer;">EVET</button>
                            <button onclick="finalizeLades('${m.id}', 'DRAW')" style="background:#f59e0b; color:black; border:none; padding:5px 12px; border-radius:5px; font-weight:bold; font-size:11px; cursor:pointer;">BERABERLİK</button>
                            <button onclick="finalizeLades('${m.id}', 'NO')" style="background:#ef4444; color:white; border:none; padding:5px 12px; border-radius:5px; font-weight:bold; font-size:11px; cursor:pointer;">HAYIR</button>
                        `;
                    }
                    adminActiveMarkets.innerHTML += `
                        <div style="background:#030814; padding:12px 15px; border-radius:8px; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap;">
                            <div style="font-size:13px; min-width:200px;"><b style="color:white;">${m.title}</b><br><span style="color:#64748b; font-size:12px;">Havuz: ${total} Token (E: ${yesPool} / H: ${noPool}${m.category === 'Spor' ? ` / B: ${drawPool}` : ''})</span></div>
        ...
        if (usersTable) {
            ...
            if (Object.keys(users).length === 0) {
                usersTable.innerHTML = `<tr><td colspan="4" style="text-align:center; color:#64748b; padding:15px;">Kayıtlı kullanıcı bulunmuyor.</td></tr>`;
            } else {
                Object.entries(users).forEach(([key, user]) => {
                    if (!user) return;
                    const isSelf = user.email === "tsulhan@gmail.com";
                    const displayName = user.nickname || maskUserEmail(user.email);
                    const deleteButtonHTML = isSelf ? `<span style="color:#64748b; font-size:11px; padding:4px 8px;">🔒 Korumalı</span>` :
                    ...
                    usersTable.innerHTML += `
                        <tr style="border-bottom:1px solid #1c2541;">
                            <td style="padding:8px 0; font-size:13px;"><strong>${displayName}</strong> ${user.isAdmin || user.email === "tsulhan@gmail.com" ? "👑" : ""}<span style="color:#64748b; font-size:11px; display:block;">${user.email}</span></td>
                            <td style="padding:8px 0; font-size:13px; color:#ff4aa2; font-family:monospace;">${user.password || "1234"}</td>
                            <td style="padding:8px 0; font-size:13px; color:#24ffff; font-weight:bold;">${(user.balance || 0).toLocaleString("tr-TR")}</td>
                            <td style="padding:8px 0; text-align:right;"><button onclick="setTokensManual('${user.email}', ${user.balance || 0})" style="background:#ff4aa2; color:white; border:none; padding:4px 10px; border-radius:4px; font-size:11px; cursor:pointer; font-weight:500;">✏️ Bakiye Düzenle</button> ${deleteButtonHTML}</td>
                        </tr>
                    `;
                });
            }
        ...
        console.error("Admin panel render hatası:", error);
    }
...
async function openTokenRequestModal() {
    ...
    try {
        ...
        const amount = prompt(`💰 MEVCUT BAKİYENİZ: ${currentBalance.toLocaleString("tr-TR")} Token\n👤 Kullanıcı: ${displayName}\n\nKaç Token talep etmek istiyorsunuz?\n(Yönetici onayı gereklidir)`);
        const tokenAmount = parseInt(amount);
        if (isNaN(tokenAmount) || tokenAmount <= 0) { alert("Geçersiz miktar!"); return; }
        const reqKey = uniqueId("req");
        await fbSet(`adminRequests/${reqKey}`, {
            id: reqKey, type: "token", email: currentUserEmail, nickname: user.nickname || currentUserEmail,
            amount: tokenAmount, currentBalance: currentBalance, status: "Bekliyor", createdAt: Date.now()
        });
        if (typeof sendNotificationToAdmins === 'function') {
            await sendNotificationToAdmins({
                title: "💰 Token Talebi!",
                message: `${displayName} (${currentUserEmail}) kullanıcısı ${tokenAmount} Token talep ediyor!`,
                type: "token_request", link: "dashboard.html?tab=admin",
...
async function sendNotificationToAllUsers(notificationData) {
    ...
    try {
        ...
        const promises = Object.keys(users).map(userKey => {
            ...
            if (user && user.email) return createNotification(user.email, notificationData);
...
async function sendBetNotificationToParticipants(marketId, bettorEmail, choice, amount, marketTitle, bettorNickname) {
    ...
    try {
        const historySnap = await fbGet("betHistory");
        const history = historySnap || {};
        const participants = Object.values(history).filter(h => h.marketId === marketId && h.email !== bettorEmail).map(h => h.email);
        const uniqueParticipants = [...new Set(participants)];
        if (uniqueParticipants.length === 0) {
            console.log("ℹ️ Bu ladese katılan başka kullanıcı yok.");
            return;
        }
        ...
        const promises = uniqueParticipants.map(email => {
            return createNotification(email, {
                ...
                type: "new_bet", marketId: marketId, link: "dashboard.html?tab=mevcut-ladesler",
...
async function getNotifications(userEmail) {
    ...
    try {
        ...
        return Object.values(notifications).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
    } catch (error) {
...
async function toggleNotificationDropdown() {
    ...
    if (!currentUser) {
        list.innerHTML = `<div style="text-align:center; color:#64748b; padding:30px; font-size:13px;">Lütfen giriş yapın.</div>`;
        return;
    }
    try {
        const notifications = await getNotifications(currentUser);
        if (notifications.length === 0) {
            list.innerHTML = `<div style="text-align:center; color:#64748b; padding:30px; font-size:13px;">📭 Bildirim bulunmuyor.</div>`;
            return;
        }
        ...
        list.innerHTML = `<div style="text-align:center; color:#ef4444; padding:20px; font-size:13px;">❌ Bildirimler yüklenirken bir hata oluştu.</div>`;
    }
}

function getNotificationTypeText(type) {
    const types = {
        'new_market': 'Yeni Lades', 'new_bet': 'Yeni Bahis',
        'closing': 'Kapanış Uyarısı', 'result': 'Sonuç',
        'token_request': 'Token Talebi', 'invite_request': 'Davet Talebi'
    };
...
async function markAllNotificationsAsRead() {
    ...
    try {
        ...
        const notifSnap = await fbGet(`notifications/${userKey}`);
        const notifications = notifSnap || {};
        const promises = Object.keys(notifications).map(key => db.ref(`notifications/${userKey}/${key}/read`).set(true));
        await Promise.all(promises);
        updateNotificationBadge();
        await toggleNotificationDropdown();
    } catch (error) {
...
function startNotificationListener() {
    ...
    const userKey = currentUserEmail.replace(/\./g, ',');
    fbRef(`notifications/${userKey}`).on("value", () => { updateNotificationBadge(); });
}
...
function toggleChatPanel() {
    ...
    if (panel.classList.contains('open')) {
        ...
        setTimeout(() => { const input = document.getElementById('chat-input'); if (input) input.focus(); }, 300);
    }
}
...
function switchChatTab(tab) {
    ...
    if (tab === 'market' && currentMarketIdForChat) {
        if (marketTab) { marketTab.style.display = 'block'; marketTab.textContent = `📊 ${activeMarketTitle || 'Lades'}`; }
    } else {
        if (marketTab) marketTab.style.display = 'none';
        if (tab !== 'market') currentMarketIdForChat = null;
    }
...
async function loadChatMessages(tab) {
    ...
    if (chatMessageListener) { chatMessageListener(); chatMessageListener = null; }
    let chatPath = 'chats/global';
    if (tab === 'market' && currentMarketIdForChat) chatPath = `chats/market_${currentMarketIdForChat}`;
    messagesContainer.innerHTML = `<div style="text-align:center; color:#64748b; padding:30px; font-size:13px;">💬 Yükleniyor...</div>`;
    ...
    chatMessageListener = fbRef(chatPath).on('value', (snapshot) => {
        ...
        if (messages.length === 0) {
            messagesContainer.innerHTML = `<div style="text-align:center; color:#64748b; padding:30px; font-size:13px;">💬 Henüz mesaj yok. İlk mesajı sen gönder!</div>`;
        ...
        messagesContainer.innerHTML = messages.map(msg => {
            ...
            return `<div class="chat-message ${isOwn ? 'sent' : 'received'}"><div class="sender">${escapeHtml(msg.sender)}</div><div class="message-text">${escapeHtml(msg.message)}</div><div class="time">${time}</div></div>`;
        }).join('');
        ...
        if (!panel || !panel.classList.contains('open')) {
            chatUnreadCount++;
            const badge = document.getElementById('chat-unread-badge');
            if (badge) { badge.textContent = chatUnreadCount; badge.style.display = 'inline-block'; }
        }
    });
}
...
async function sendChatMessage() {
    ...
    let chatPath = 'chats/global';
    if (currentChatTab === 'market' && currentMarketIdForChat) chatPath = `chats/market_${currentMarketIdForChat}`;
    const messageId = uniqueId('msg');
    await fbSet(`${chatPath}/${messageId}`, { sender: displayName, senderEmail: currentUser, message: message, timestamp: Date.now() });
    input.value = '';
    input.focus();
}
...
function openMarketChat(marketId, marketTitle) {
    ...
    const panel = document.getElementById('chat-panel');
    const toggleBtn = document.getElementById('chat-toggle-btn');
    if (!panel.classList.contains('open')) { panel.classList.add('open'); if (toggleBtn) toggleBtn.classList.add('hidden'); }
    const marketTab = document.getElementById('market-chat-tab');
    if (marketTab) { marketTab.style.display = 'block'; marketTab.textContent = `📊 ${marketTitle}`; marketTab.classList.add('active'); }
...
function closeMarketChat() {
    ...
    const marketTab = document.getElementById('market-chat-tab');
    if (marketTab) { marketTab.style.display = 'none'; marketTab.classList.remove('active'); }
...
function startChatSystem() {
    ...
    const panel = document.getElementById('chat-panel');
    if (panel) panel.addEventListener('transitionend', () => { if (panel.classList.contains('open')) { const input = document.getElementById('chat-input'); if (input) input.focus(); } });
    setTimeout(() => { loadChatMessages('global'); }, 500);
...
async function initProfilePage() {
    ...
    try {
        const [marketsSnap, historySnap] = await Promise.all([fbGet("customMarkets"), fbGet("betHistory")]);
        renderProfileBets(currentUserEmail, marketsSnap || {}, historySnap || {});
    } catch (err) { console.error("Profil verileri yüklenirken hata:", err); }
}
...
function renderProfileBets(currentUserEmail, markets, history) {
    ...
    const allBets = Object.values(history).filter(b => b && b.email === currentUserEmail).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
        ...
        if (market.status === "Aktif") {
            ...
            activeContainer.innerHTML += `
                <div class="user-bet-card">
                    <div class="bet-details"><h3>${market.title}</h3><p>Kategori: <b>${market.category || "Genel"}</b> • Bitiş: ${market.date || "-"}</p><div style="margin-top:8px;">${choiceBadge}</div></div>
                    <div class="bet-stats">
                        <div class="stat-box"><div class="lbl">Sizin Yatırımınız</div><div class="val" style="color:#24ffff;">${(bet.amount || 0).toLocaleString("tr-TR")} Token</div></div>
                        <div class="stat-box" style="border-left:1px solid #1c2541; padding-left:20px;"><div class="lbl">Toplam Havuz</div><div class="val" style="color:white;">${totalPool.toLocaleString("tr-TR")} Token</div></div>
                    </div>
                </div>
            `;
        } else {
            pastCount++;
            let resultHTML = "", winningChoice = "";
            ...
            pastContainer.innerHTML += `
                <div class="user-bet-card" style="border-color:#161b2c; background:#060b19;">
                    <div class="bet-details"><h3 style="color:#94a3b8;">${market.title}</h3><p>Kategori: ${market.category || "Genel"} • Durum: <span style="color:#ff4aa2; font-weight:600;">Sonuçlandı</span></p><div style="margin-top:8px;">${choiceBadge}</div></div>
                    <div class="bet-stats">
                        <div class="stat-box"><div class="lbl">Yatırdığınız</div><div class="val" style="color:#94a3b8;">${(bet.amount || 0).toLocaleString("tr-TR")} Token</div></div>
                        <div class="stat-box" style="border-left:1px solid #1c2541; padding-left:20px; min-width:140px;"><div class="lbl">Sonuç</div><div class="val">${resultHTML}</div></div>
                    </div>
                </div>
            `;
        }
    });
    if (activeCount === 0) activeContainer.innerHTML = `<div style="text-align:center; color:#64748b; padding:30px; font-size:14px;">Şu an aktif ladesiniz bulunmuyor.</div>`;
    if (pastCount === 0) pastContainer.innerHTML = `<div style="text-align:center; color:#64748b; padding:30px; font-size:14px;">Henüz sonuçlanan lades geçmişiniz yok.</div>`;
}